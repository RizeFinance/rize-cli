...
class TransferController {
    ...
    async createTransfer(customerExternalUid, data) {
        ...
        const transferTransactions = rizeTransactions.data
            ...
            .map(x => {
                delete x.custodial_account_uids;
                delete x.transaction_event_uids;
        ...
        const [sourceRizeSyntheticAccount, destinationRizeSyntheticAccount] = await Promise.all([
        ...
        // Update the DB
        await Promise.all([
            this._syntheticAccountService.updateSyntheticAccountInDb(sourceRizeSyntheticAccount),
            this._syntheticAccountService.updateSyntheticAccountInDb(destinationRizeSyntheticAccount)
        ]);
