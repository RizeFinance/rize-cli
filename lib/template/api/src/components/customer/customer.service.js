   ...
   */
    ...
    mapRizeCustomerToDbCustomer(rizeCustomer) {
        const customerKeys = [
            ...
            { to: 'business_name', from: 'details.business_name' },
        ];

        return mapObject(rizeCustomer, customerKeys);
    }

    async createRizeCustomer(external_uid, email, customer_type) {
        return await this._rizeProvider.customer.create(
            external_uid,
            email,
            customer_type
        );
    ...
    async updateCustomerInRize(customerUid, customerEmail, customerDetails) {
        return await this._rizeProvider.customer.update(
            customerUid,
            customerEmail,
            customerDetails
        );
    ...
    async getCustomerProducts(customerUid) {
        return await this._rizeProvider.customerProduct.getList({
            customer_uid: [customerUid],
        });
    }

    async createCustomerProduct(customerUid, productUid) {
        return await this._rizeProvider.customerProduct.create(
            customerUid,
            productUid
        );
    }
    ...
    async answerProfileQuestions(customerUid, answers) {
        return await this._rizeProvider.customer.updateProfileAnswers(
            customerUid,
            answers
        );
    }

    async verifyCustomer(customerUid) {
        return await this._rizeProvider.customer.identityConfirmation(customerUid);
    }
}
