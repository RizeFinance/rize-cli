...
module.exports = function CustomerRouter(customerController, authGuard, customerValidator) {
    ...
    router.post('/', authGuard, customerValidator.validateCreateCustomer, (req, res, next) => {
        const {
            email,
            customer_type,
            product_uid
        ...
        customerController.createCustomerWithWorkflow(req.user.sub, email, customer_type, product_uid)
            .then((data) => {
                res.status(200).send(data);
            })
            .catch((err) => {
    ...
    router.put('/verify', authGuard, (req, res, next) => {
        customerController.verifyCustomer(req.user.sub)
            .then((data) => {
                res.status(200).send(data);
            })
            .catch((err) => {
