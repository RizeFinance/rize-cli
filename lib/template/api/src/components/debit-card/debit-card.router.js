...
module.exports = function DebitCardRouter(
    ...
    router.put('/:uid/reissue', authGuard, (req, res, next) => {
    ...
    router.put('/:uid/unlock', authGuard, debitCardValidator.validateLock, (req, res, next) => {
        const { uid } = req.params;
        const { reason } = req.body;

        debitCardController.unlockDebitCardByUid(req.user.sub, uid, reason)
            .then((data) => {
                res.status(200).send(data);
            })
            .catch((err) => {
