    ...
    */
    ...
    mapRizeDebitCardToDbDebitCard (rizeDebitCard) {
        const debitCardKeys = [
            ...
            { to: 'card_last_four_digit', from: 'card_last_four_digits' },
    ...
    async activateDebitCard (debitCardUid, cardLastFourDigits, cvv, expiryDate) {
        return await this._rizeProvider.debitCard.activate(debitCardUid, cardLastFourDigits, cvv, expiryDate);
    }    

    async getPinSetToken (debitCardUid) {
        return await this._rizeProvider.debitCard.getPinChangeToken(debitCardUid);
    }

    async getAccessToken(debitCardUid) {
        return await this._rizeProvider.debitCard.getAccessTokenData(debitCardUid);
    }

    async getVirtualCardImage(configId, token) {
        return await this._rizeProvider.debitCard.getVirtualCardImage(configId, token);
    }

    async migrateVirtualCard(uid, customerUid, poolUid) {
        return await this._rizeProvider.debitCard.migrateVirtualCard({ uid, customerUid, poolUid });
    }
}
