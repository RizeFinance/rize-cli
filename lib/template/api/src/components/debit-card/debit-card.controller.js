...
class DebitCardController {
    ...
    async unlockDebitCardByUid (customerExternalUid, debitCardUid, unlockReason) {
        // Check if the customer is associated with the debit card
        const allowed = await this._debitCardService.getDebitCardByUid(
            debitCardUid,
            customerExternalUid
        ...
        const unlockedDebitCard = await this._debitCardService.unlockRizeDebitCard(debitCardUid, unlockReason);
