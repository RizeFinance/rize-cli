...
const promptSelectAccountAuthProvider = async options => {
    const defaultAuthProvider = 'Stripe';
    const answers = await inquirer.prompt([{
        type: 'list',
...
const promptStripeKeys = async options => {
    const questions = [{
        name: 'accountAuthClientID',
        type: 'input',
        message: 'Please input your Stripe Client ID',
        default: ''
    }, {
        name: 'accountAuthSecret',
        type: 'input',
        message: 'Please input your Stripe Secret',
        default: ''
    }];
...
const promptAccountAuthOptions = async options => {
    ...
    switch (options.accountAuthProvider) {
        case 'Stripe':
            options = await promptStripeKeys(options);
            break;
    }
