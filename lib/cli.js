...
const promptSetupAuth = async options => {
    const answers = await inquirer.prompt([{
        name: 'setupAuth',
        type: 'confirm',
        message: 'Would you like to setup the authentication provider? (Recommended)',
        default: true
    }]);
...
const createEnv = async (options) => {
    ...
    const object = {
        ...
        DB_HOST: `${projectName}-db-service`,
        DB_DATABASE: projectName,
        DB_DEFAULT_SCHEMA: options.database === 'MySQL' ? projectName : 'core',
        DB_USER: 'root',
        DB_PASSWORD: 'password',
        DATA_MIGRATIONS_HOST: `${projectName}-migrations-service`,
