...
const promptForMissingOptions = async options => {
    if (options.action === 'create') {
        ...
        const rizeQuestions = [{
            ...
            name: 'rizeDefaultProductUid',
            type: 'input',
            message: 'Please input your programs default product UID',
            default: ''
        }, {
            name: 'rmqHosts',
            type: 'input',
            message: 'Please input your Rize Message Queue host (comma-dilimited if more then one)',
            default: 'mq-sandbox.rizefs.com'
        }, {
            name: 'rmqTopic',
            type: 'input',
...
exports.start = async args => {
    program
        .name('rize')
        .description('A CLI to provision the necessary components to run a banking application using Rize')
        .version('1.1.2');

    program.command('create')
        .description('Create a new rize boilerplate application')
        .argument('Project Name <string>', 'Name of project directory')
        .option('--db, --database', 'Create a database for your app')
        .option('--auth, --authentication', 'Setup the authentication provider for your app.')
        .action(async () => {
